...
if [ -e /proc/device-tree/model ]; then
	PROJECT_MODEL=$(cat /proc/device-tree/model)
else
	PROJECT_MODEL="PWRT"
fi
...
rand_mac() {
 MACADDRPREF=`od /dev/urandom -N5 -t x1 -An | sed 's/ //g'`
 MACADDRFULL=00$MACADDRPREF
 echo $MACADDRFULL
}
#NTmore added, mac address write
MAC_ADDR=`uci get /etc/config/wifi.wifi.macaddress | sed -e "s/://g"`
MAC_ADDR_SEC=`uci get /etc/config/wifi-second.wifi.macaddress | sed -e "s/://g"`

if [ $MAC_ADDR == "000000000000" ];then
	WLAN0MAC=$(rand_mac)
	#Random mac address for temp
	echo "$WLAN0MAC" > $CONFIG_DIR/nic0_addr
	echo "$(rand_mac)" > $CONFIG_DIR/nic1_addr
	echo "$WLAN0MAC" > $CONFIG_DIR/wlan0_addr
	echo "$(rand_mac)" > $CONFIG_DIR/wlan1_addr
	echo "$(rand_mac)" > $CONFIG_DIR/wlan2_addr
	echo "$(rand_mac)" > $CONFIG_DIR/wlan3_addr
	echo "$(rand_mac)" > $CONFIG_DIR/wlan4_addr
	echo "$(rand_mac)" > $CONFIG_DIR/wlan5_addr
	echo "$(rand_mac)" > $CONFIG_DIR/wlan6_addr
	echo "$(rand_mac)" > $CONFIG_DIR/wlan7_addr
else #Mac address in Factory files.
	#NTmore added, mac address write
	echo "$MAC_ADDR" > $CONFIG_DIR/nic0_addr
	echo "$MAC_ADDR_SEC" > $CONFIG_DIR/nic1_addr
	echo "$MAC_ADDR" > $CONFIG_DIR/wlan0_addr
	echo "$MAC_ADDR_SEC" > $CONFIG_DIR/wlan1_addr
...
if [ $PROJECT_MODEL == "NTLD-200" ];then
if [ $1 == "wlan0" ];then
...
elif [ $PROJECT_MODEL == "NTLR-310" ];then
if [ $1 == "wlan0" ];then
  HW_VER=`uci get /nvm/bsp/factory.value.hw_ver`

  if [ $HW_VER == '1.0' ]
  then
        #echo 'Hardware Revision is 1.0'
        echo "222121201F1F2121222222222222" > $CONFIG_DIR/tx_power_cck_a
        echo "222121201F1F2121222222222222" > $CONFIG_DIR/tx_power_cck_b
        echo "2827252424252526272828282828" > $CONFIG_DIR/tx_power_ht40_1s_a
        echo "2827252424252526272828282828" > $CONFIG_DIR/tx_power_ht40_1s_b
        echo "0000000000000000000000000000" > $CONFIG_DIR/tx_power_diff_ht40_2s
        echo "0000000000000000000000000000" > $CONFIG_DIR/tx_power_diff_ht20
        echo "0000000000000000000000000000" > $CONFIG_DIR/tx_power_diff_ofdm
...
elif [ $PROJECT_MODEL == "NTLR-210" ];then
if [ $1 == "wlan0" ];then
	TEMP_BOARDREV=`uci get /nvm/bsp/factory.value.last_bsp_update_date`
	if [ $TEMP_BOARDREV -ge 20161212 ];then
	...
	else #for old NTLR-210 revision 2 board
        	echo "22222121212121202020201E1E1E" > $CONFIG_DIR/tx_power_cck_a
        	echo "22222121212121202020201E1E1E" > $CONFIG_DIR/tx_power_cck_b
        	echo "2827262626262625262626232323" > $CONFIG_DIR/tx_power_ht40_1s_a
        	echo "2827262626262625262626232323" > $CONFIG_DIR/tx_power_ht40_1s_b
	fi
        echo "0000000000000000000000000000" > $CONFIG_DIR/tx_power_diff_ht40_2s
        echo "0000000000000000000000000000" > $CONFIG_DIR/tx_power_diff_ht20
        echo "0000000000000000000000000000" > $CONFIG_DIR/tx_power_diff_ofdm
...
elif [ $PROJECT_MODEL == "NTLR-220" ];then
if [ $1 == "wlan0" ];then
	TEMP_BOARDREV=`uci get /nvm/bsp/factory.value.last_bsp_update_date`
	if [ $TEMP_BOARDREV -ge 20161212 ];then
	        echo "1F1E1D1D1D1D1D1C1C1C1C1E1E1E" > $CONFIG_DIR/tx_power_cck_a
...
elif [ $PROJECT_MODEL == "NTLO-510" ];then
if [ $1 == "wlan0" ];then
	echo "201F1F202121201F1E1F221E1E1E" > $CONFIG_DIR/tx_power_cck_a
	echo "212120201F2122221E1D201E1E1E" > $CONFIG_DIR/tx_power_cck_b
	echo "2322222323232222222324232323" > $CONFIG_DIR/tx_power_ht40_1s_a
...
if [ $1 == "wlan0" ];then
	echo "NTM3800PortableRouter" > $CONFIG_DIR/ssid
elif [ $1 == "wlan0-va0" ];then
	echo "NTM3800PortableRouter_2nd" > $CONFIG_DIR/ssid
fi
if [ $1 == "wlan0" ];then
...
if [ $PROJECT_MODEL == "NTLR-210" ] || [ $PROJECT_MODEL == "NTLR-220" ] || [ $PROJECT_MODEL == "NTLR-310" ]|| [ $PROJECT_MODEL == "NTLO-510" ]; then
echo "0" > $CONFIG_DIR/wsc_disabled
echo "2" > $CONFIG_DIR/wsc_method
elif [ $PROJECT_MODEL == "NTLD-200" ]; then
...
LOCAL_IP_ADDR=`uci get /etc/config/lte-gw.local_param.local_ip_addr`
echo $LOCAL_IP_ADDR > $CONFIG_ROOT_DIR/ip_addr
LOCAL_MASK_ADDR=`uci get /etc/config/lte-gw.local_param.local_ip_mask`
echo $LOCAL_MASK_ADDR > $CONFIG_ROOT_DIR/net_mask
...
echo "CLM-C324" > $CONFIG_ROOT_DIR/device_name
elif [ $PROJECT_MODEL == "NTLR-310" ];then
