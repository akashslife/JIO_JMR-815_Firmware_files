...
function getindexData()
{
	...
	dhtmlxAjax.get("/index_ajax.cgi?init=0&"+timestamp, updateINDEXData);
}

function updateINDEXData(loader)
{
	try
	{
		var data = eval("(" + loader.xmlDoc.responseText + ")");
    login_on = data.Logon;
    document.getElementById("out_token").value = data.token;
    if(login_on==0) {
      document.getElementById("login_status").src="../../image/mainTop/Login.jpg";
    }
		else {
      document.getElementById("login_status").src="../../image/mainTop/Logout.jpg";
    }		
    if (typeof(data.INDEX) != "undefined")
    {
      modelname = data.INDEX.model;
      opmode = data.INDEX.op;
      top.webbody.location=data.INDEX.url;
    }
    start_session();
...
function beforeload(){
	if(document.getElementById("bodyid") != undefined){
...
function afterload(){
	if(document.getElementById("bodyid") != undefined){
		...
		document.getElementById("bodyid").style.display = display_on();			
	}		
}
function display_on()
{
...
function log_out() { 
  var csrf = document.getElementById("out_token").value;
...
function ajaxDone(req) { 
  if(req.xmlDoc.readyState == 4) { 
    try { 
      var res = eval("(" + req.xmlDoc.responseText + ")");
      if(res.status =="logoff") { 
        alert('Session logout because another user also login');  
        log_out();
      }
...
function ajaxstart() { 
  var timestamp = Number(new Date()); 
  dhtmlxAjax.get("/sess.cgi?"+timestamp, ajaxDone);
}
...
function log_in() { 
  top.webbody.location ="login.cgi"; 
}
...
function namosw_init_list(top_layer) { 
  ...
  if(opmode!=="G")
    l1.additem('Connection URL', 'connection_url.cgi', 'parent.webbody'); 
  l2 = new namosw_list(layer, true, 225, 35, '2', '#50B6BF', '#FFFFFF', 0, '#FFF799');
  l2.additem('LTE Information', 'LTE_info.cgi', 'parent.webbody');
  l2.additem('LAN Information', 'LAN_info.cgi', 'parent.webbody');
  l2.additem('WAN Information', 'WAN_info.cgi', 'parent.webbody');
  l2.additem('Device Details', 'Device_info.cgi', 'parent.webbody'); 
  l2.additem('System Performance', 'System_performance.cgi', 'parent.webbody'); 
  l1.addlist(l2, 'Status Information', '', 'parent.webbody'); 

  if(login_on==1) {
    l9 = new namosw_list(layer, false, 225, 35, '2', '#50B6BF', '#FFFFFF', 0, '#FFF799');
    l9.additem('&nbsp;&nbsp;&nbsp;APN', 'LTE_setting.cgi', 'parent.webbody');
    l9.additem('&nbsp;&nbsp;&nbsp;PIN', 'PIN_Management.cgi', 'parent.webbody');
    l3 = new namosw_list(layer, true, 225, 40, '2', 'white', '#50B6BF', 0, '#8E61BC');
    l3.addlist(l9, '&nbsp;LTE', '', 'parent.webbody');
    l6 = new namosw_list(layer, false, 225, 35, '2', '#50B6BF', '#FFFFFF', 0, '#FFF799');
    l6.additem('&nbsp;&nbsp;&nbsp;Network Mode', 'Basic.cgi', 'parent.webbody');
    l6.additem('&nbsp;&nbsp;&nbsp;LAN', 'DHCP_setting.cgi', 'parent.webbody'); 
    l6.additem('&nbsp;&nbsp;&nbsp;Wi-Fi Setting', 'Security_Mode.cgi', 'parent.webbody');
    l6.additem('&nbsp;&nbsp;&nbsp;Wi-Fi MAC Filter', 'MAC_Filter.cgi', 'parent.webbody');
    if((modelname!="JDR810")&&(modelname!="NTLO510"))
      l6.additem('&nbsp;&nbsp;&nbsp;WPS', 'WPS.cgi', 'parent.webbody');
    l6.additem('&nbsp;&nbsp;&nbsp;Gaming & Apps Management', 'Port_Forward_Setting.cgi', 'parent.webbody');
    l3.addlist(l6, '&nbsp;Network', '', 'parent.webbody');
    if(modelname == "JMR815" || modelname == "NTLR310")
      l3.additem('&nbsp;Storage', 'Storage_Setting.cgi', 'parent.webbody'); 
    l7 = new namosw_list(layer, false, 225, 35, '2', '#50B6BF', '#FFFFFF', 0, '#FFF799');
    l7.additem('&nbsp;&nbsp;&nbsp;Account', 'Change_ID_PW.cgi', 'parent.webbody');
    l7.additem('&nbsp;&nbsp;&nbsp;Device Settings', 'Device_setting.cgi', 'parent.webbody');
    l7.additem('&nbsp;&nbsp;&nbsp;Restore Default', 'Factory_reset.cgi', 'parent.webbody');
    l3.addlist(l7, '&nbsp;User Management', '', 'parent.webbody');
    l8 = new namosw_list(layer, false, 225, 35, '2', '#50B6BF', '#FFFFFF', 0, '#FFF799');
    l8.additem('&nbsp;&nbsp;&nbsp;Software', 'Firmware_Upgrade.cgi', 'parent.webbody');
    l3.addlist(l8, '&nbsp;Firmware Management', '', 'parent.webbody');
    l1.addlist(l3, 'Settings', '', 'parent.webbody'); 
  }
  l1.make(0, 14); 
} 
