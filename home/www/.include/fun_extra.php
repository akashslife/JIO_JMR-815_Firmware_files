...
//------------------------------------------------------------------------------
function make_list_link($_action,$_filt=NULL,$_srt=NULL) {
						// make link to next page
	if($_action=="" || $_action==NULL) $_action="list";
...
function get_file_size($dir, $item) {		// file size
	return getfilesize(get_abs_item($dir, $item));
}
//------------------------------------------------------------------------------
...
function remove($item) {			// remove file / dir
	...
	elseif(@is_dir($item)) {
		if(($handle=@opendir($item))===false) show_error($item.": ".$GLOBALS["error_msg"]["opendir"]); //	show_error(basename($item).": ".$GLOBALS["error_msg"]["opendir"]);

		while(($file=readdir($handle))!==false) {
			if(($file==".." || $file==".")) continue;
			
			$new_item = $item."/".$file;
			if(!@file_exists($new_item)) show_error($item.": ".$GLOBALS["error_msg"]["readdir"]); //show_error(basename($item).": ".$GLOBALS["error_msg"]["readdir"]);
...
//------------------------------------------------------------------------------
function Generation_csrf() {
	//jk need to random on the device
	$bytesA = mt_rand(11111,99999);
	$bytesB = mt_rand(11111,99999);
	$GLOBALS['__SESSION']['token'] = $bytesA.$bytesB;
}
//------------------------------------------------------------------------------
function Verify_csrf($tok) {
	if(isset($GLOBALS['__SESSION']['token']) && $tok == $GLOBALS['__SESSION']['token'])
		return 1;
	else 
		return 0;
}	
