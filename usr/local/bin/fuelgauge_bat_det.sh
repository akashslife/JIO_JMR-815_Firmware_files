#!/bin/sh
#
#  Battery Detection Fuel Gauge BQ27426G1.
#
#
...
if [ $PROJECT_TYPE != "PWRT" -o $ALTAIR_HW_COMPAT != "1" ]; then 
	echo "not Altair Compatible PWRT skipping script "$0 > /dev/kmsg       
	exit 0
fi
...
while [ $BAT_DET -eq 0 ];do
  if [ $i -eq 500 ]; then
    echo "Fuel Gauge bat_det set error"
    echo "Fuel Gauge bat_det set error" > /dev/kmsg  
    exit
  fi
  let i+=1
  FLAGS_STATUS=$(i2cget -y $FG_I2C_BUS $FG_CHIPID 0x06 w)
  BAT_DET=$((($FLAGS_STATUS & 0x0008)>>3))
  usleep 200000
  echo "Waiting to Begin BAT_DET mode"
done 

echo "Begin BAT_DET mode"
