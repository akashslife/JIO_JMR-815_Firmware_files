...
setup_interface_static() {
	...
	do_sysctl "net.ipv6.conf.$iface.disable_ipv6" 0
	do_sysctl "net.ipv6.conf.$iface.accept_ra" 2
	do_sysctl "net.ipv6.conf.$iface.forwarding" 0
...
setup_interface() {
	...
	case "$proto" in
		...
		dhcp)
			...
			$DEBUG eval udhcpc -t 5 -T 1 -i "$iface" \
				${ipaddr:+-r $ipaddr} \
				${hostname:+-H $hostname} \
				${clientid:+-c $clientid} \
				${vendorid:+-V $vendorid} \
